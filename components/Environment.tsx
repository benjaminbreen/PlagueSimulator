    ...
    <group>
      ...
      {visibleBuildings.map((data) => (
        <Building
          ...
          selectionEnabled={selectionEnabled}
          isSelected={selectionEnabled && selectedBuildingId === data.id}
          onSelectBuilding={onSelectBuilding}
          torchIntensity={torchIntensity}
          district={district}
          nightFactor={nightFactor}
...
export const Environment: React.FC<EnvironmentProps> = ({ mapX, mapY, sessionSeed = 0, onGroundClick, onBuildingsGenerated, onClimbablesGenerated, onHeightmapBuilt, onTreePositionsGenerated, nearBuildingId, timeOfDay, enableHoverWireframe = false, enableHoverLabel = false, selectionEnabled = false, selectedBuildingId = null, onSelectBuilding, pushables = [], fogColor, heightmap, laundryLines = [], hangingCarpets = [], catPositionRef, ratPositions, npcPositions, playerPosition, isSprinting, showCityWalls = true, occludedBuildingIds = [] }) => {
  ...
  const handleBuildingsGenerated = React.useCallback((buildings: BuildingMetadata[]) => {
    ...
    for (const climbable of allClimbables) {
      if (climbable.type === 'WOODEN_LADDER') {
        ...
        if (building) {
          ...
          const hatchYOffset = district === 'WEALTHY' ? 0.32 : 0;
          climbable.roofY = finalHeight + hatchYOffset;
          if (isMultiStory && !building.hasRoofHatch) {
            const { position } = calculateRooftopHatchPosition(climbable);
            building.hasRoofHatch = true;
            building.roofHatchWorldPos = [position[0], finalHeight + hatchYOffset, position[2]];
          }
        }
      }
    }
